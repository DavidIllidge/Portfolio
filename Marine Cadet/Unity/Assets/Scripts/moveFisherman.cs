...
public class moveFisherman : MonoBehaviour {
	...
	void getNewPosition()
	{
		if (FishHookObject.transform.position.y == -0.98f)
		{
			...
			if (newHook == true)
			{
				newHook = false;
				timer = 0;
				newPosition = new Vector3((Random.Range(-6.0f, 6.0f)), 0.25f, transform.position.z);
				extended = false;
		...
		if (extended == false)
		{
			...
			if (currentPosition == newPosition)
			{
				...
				extended = true;
				//call extendedHook
				FishHookObject.GetComponent<MoveHook>().extendHook = true;
				FishHookObject.GetComponent<MoveHook>().getNewHookPosition = true;
				newHook = true;
